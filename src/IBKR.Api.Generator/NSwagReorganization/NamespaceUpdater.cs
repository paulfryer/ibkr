...
public class NamespaceUpdater
{
    ...
    private async Task<bool> UpdateFileNamespace(string filePath, string folder)
    {
        ...
        {
            "Models" => "IBKR.Api.NSwag.Models",
            "Services" => "IBKR.Api.NSwag.Services",
            "Interfaces" => "IBKR.Api.NSwag.Interfaces",
            "Clients" => "IBKR.Api.NSwag.Clients",
            "Helpers" => "IBKR.Api.NSwag.Helpers",
            _ => "IBKR.Api.NSwag"
        };

        // Update namespace declaration (handle both old-style and new-style)
        // Old style: namespace IBKR.Api.NSwag { ... }
        // New style: namespace IBKR.Api.NSwag;
        // Match both the old temp namespace and any existing namespace
    ...
    private string AddUsingDirectives(string content, string folder)
    {
        ...
        if (folder == "Services" || folder == "Interfaces")
        {
            usingsToAdd.Add("using IBKR.Api.NSwag.Models;");
        }

        // Services need Interfaces
        if (folder == "Services")
        {
            usingsToAdd.Add("using IBKR.Api.NSwag.Interfaces;");
        }
        ...
        if (folder == "Models")
        {
            ...
            {
                usingsToAdd.Add("using IBKR.Api.NSwag.Helpers;");
            }
        }
